...
def store_valid_passwords(folder, id_base, password_base, salt_base, recurse = true)
  ...
  (1..3).each do |num|
    ...
    call_function('simplib::passgen::simpkv::set', full_id, prev_password, prev_salt, complexity, complex_only)

    call_function('simplib::passgen::simpkv::set', full_id, current_password, current_salt, complexity, complex_only)
...
describe 'simplib::passgen::simpkv::list' do
  ...
  context 'successes' do
    context 'root folder' do
      ...
      it 'returns empty password and folder results when the root folder is empty' do
        ...
        call_function('simplib::passgen::simpkv::set', 'my_id', 'password', 'salt', 1, true)
        call_function('simplib::passgen::simpkv::remove', 'my_id')
      ...
      it 'skips bad entries' do
        ...
        call_function('simpkv::put', key_without_meta, value)
    ...
    context 'sub-folder' do
      ...
      it 'returns empty password and folder results when the sub-folder is empty' do
        ...
        # passwords
        subject # rubocop:disable RSpec/NamedSubject
        call_function('simplib::passgen::simpkv::set', "#{sub_folder}/my_id", 'password', 'salt', 1, true)
        call_function('simplib::passgen::simpkv::remove', "#{sub_folder}/my_id")
      ...
      it 'returns password info and folders when sub-folder is not empty' do
        subject # rubocop:disable RSpec/NamedSubject
        expected = store_valid_passwords(sub_folder, id_base, password_base, salt_base)
