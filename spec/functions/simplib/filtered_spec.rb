            ...
            context name do
              if value['all_results'] == true
                it 'returns all results' do
                  result = subject.execute(shared_options.dup.merge({ 'path' => data['path'], 'filter' => value['filter'] }), Puppet::Pops::Lookup::Context.new('rp_env', 'simplib')) # rubocop:disable RSpec/NamedSubject
                  expect(result).to eql(data['retval'])
                end
              end
              if value['all_results'] == false
                it 'does not return all results' do
                  result = subject.execute(shared_options.dup.merge({ 'path' => data['path'], 'filter' => value['filter'] }), Puppet::Pops::Lookup::Context.new('rp_env', 'simplib')) # rubocop:disable RSpec/NamedSubject
                  if data['retval'] == {}
                    expect(result).to eql(data['retval'])
                  else
                    expect(result).not_to eql(data['retval'])
                  end
                end
              end
            end
