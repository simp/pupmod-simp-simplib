    ...
    context 'invalid config' do
      it 'rejects a setting that is too long' do
        is_expected.to run.with_params('kernel.core_pattern', '/var/core/' + 'x' * 120).and_raise_error(
         %r{Values for kernel.core_pattern must be less than 129 characters},
       )
      end
      ...
      it 'rejects a setting that is pipe to an executable that does not have a fully-qualified path' do
        is_expected.to run.with_params('kernel.core_pattern', '| my_core_dump_program %p').and_raise_error(
         %r{Piped commands for kernel.core_pattern must have an absolute path},
       )
      end
    end
  end
    ...
    context 'invalid config' do
      ...
      invalid_values.each do |value|
        it "rejects #{value}" do
          is_expected.to run.with_params('fs.inotify.max_user_watches', value).and_raise_error(
            %r{fs.inotify.max_user_watches cannot be #{Regexp.escape(value.to_s)}},
          )
        end
      end
      ...
      context 'x86_64' do
        it 'rejects items that exceed the memory limit' do
          is_expected.to run.with_params('fs.inotify.max_user_watches', 20_480).and_raise_error(
            %r{fs.inotify.max_user_watches set to 20480 would exceed system RAM},
          )
        end
      end
      ...
      context 'i686' do
        ...
        it 'rejects items that exceed the memory limit' do
          is_expected.to run.with_params('fs.inotify.max_user_watches', 40_960).and_raise_error(
            %r{fs.inotify.max_user_watches set to 40960 would exceed system RAM},
          )
        end
      end
    end
