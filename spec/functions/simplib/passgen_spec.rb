...
def test_hashes
  ...
  # This works because this fact is part of this module
  unless Facter.fact(:fips_enabled).value
    valid_hashes['md5'] = {
      "code" => '1',
      "bits" => 128,
...
describe 'simplib::passgen' do
  cases.each do |description,settings|
    ...
    context 'basic password generation' do
      it 'should run with default parameters' do
        expect { subject.execute('spectest', settings)}.to_not raise_error
        result = subject.execute('spectest', settings)

        unless settings['libkv']
          vardir = Puppet[:vardir]
      ...
      it 'should return a password that only contains "safe" special characters if complexity is 1 and complex_only is true' do
        result = subject.execute('spectest', settings.merge({'complexity' => 1, 'complex_only' => true}))
        expect(result.length).to eql(32)
      ...
      it 'should return a password that contains all special characters if complexity is 2' do
        result = subject.execute('spectest', settings.merge({'complexity' => 2}))
        expect(result.length).to eql(32)
        expect(result).to match(/(#{default_chars.join('|')})/)
      ...
      it 'should return a password that only contains all special characters if complexity is 2 and complex_only is true' do
        result = subject.execute('spectest', settings.merge({'complexity' => 2, 'complex_only' => true}))
        expect(result.length).to eql(32)
    ...
    context 'password generation with history' do
      unless settings['libkv']
        it 'should return the next to last created password if "last" is true' do
          first_result = subject.execute('spectest', settings.merge({'length' => 32}))
          second_result = subject.execute('spectest', settings.merge({'length' => 33}))
          third_result = subject.execute('spectest', settings.merge({'length' => 34}))
          expect(subject.execute('spectest', settings.merge({'last' => true}))).to eql(second_result)
        end

        it 'should return the current password if "last" is true but there is no previous password' do
          result = subject.execute('spectest', settings.merge({'length' => 32}))
          expect(subject.execute('spectest', settings.merge({'last' => true}))).to eql(result)
        end

        it 'should return current password if modifier_hash is nil' do
          result = subject.execute('spectest', settings.merge({'length' => 32}))
          expect(subject.execute('spectest', settings)).to eql(result)
        end
    ...
    context 'password encryption ' do
      ...
      test_hashes.each do |hash_selection, object|
        context "when hash == #{hash_selection}" do
          ...
          it 'should parse as modular crypt' do
            result = subject.execute('spectest', settings.merge(shared_options));
            expect(parse_modular_crypt(result)).to_not eql(nil)
          end

          it "should use #{object['code']} as the algorithm code" do
            value = parse_modular_crypt(subject.execute('spectest', settings.merge(shared_options)));
          ...
          it "should return exactly #{object['end_hash']} when salt and password are specified" do
            result = subject.execute('spectest', settings.merge(specific_options))
    ...
    context 'misc errors' do
      it 'fails when keydir cannot be created' do
        ...
        if not settings['libkv']
          is_expected.to run.with_params('spectest', settings).and_raise_error(
            /simplib::passgen: Could not make directory/
          )
        else
          is_expected.to_not run.with_params('spectest', settings).and_raise_error(
            /simplib::passgen: Could not make directory/
          )
        end
    ...
    context 'with modifier_hash parameter errors' do
      it do
        is_expected.to run.with_params('spectest', settings.merge({'length' => 'oops'})).and_raise_error(
          /Error: Length 'oops' must be an integer/)
      end

      it do
        is_expected.to run.with_params('spectest', settings.merge({'complexity' => 'oops'})).and_raise_error(
          /Error: Complexity 'oops' must be an integer/)
      end

      it do
        is_expected.to run.with_params('spectest', settings.merge({'hash' => 'sha1'})).and_raise_error(
