      ...
      context  "on #{os}" do
        ...
        context 'bugfix1049656' do
          ...
          it do
            ...
            catalog_os_ver = catalogue
              .instance_variable_get('@in_to')
              .to_s
              .split('}, ')
              .select{|x| x=~ /xxx/ }
              .first
              .sub(%q(Notify[xxx]{:name=>"xxx", :message=>"::operatingsystemmajrelease = '),'')
              .sub(/'.*$/,'')

            puts "       === catalog:    '#{catalog_os_ver}'"
