...
describe 'tpm', :type => :fact do
  ...
  let(:tpm_dir) { File.join(files_dir, 'tpm') }
  ...
  context 'tpm is enabled and unowned' do
    ...
    it 'should be a structured fact with the prescribed structure' do
      ...
      fact = Facter::TPM::Util.new(tpm_dir)
  ...
  describe Facter::TPM::Util do
    ...
    before(:each) do
      Facter::Core::Execution.stubs(:execute).with('tpm_version', :timeout => 15).returns File.read(File.join(tpm_dir, 'tpm_version.txt'))


      @tpm_fact = Facter::TPM::Util.new(tpm_dir)
    end
    ...
    describe '.get_pubek_owned' do
      context 'with a well-known owner password' do
        it 'should return the results from the tpm_getpubek command' do
          out = File.read(File.join(tpm_dir, 'tpm_getpubek.txt'))
      ...
      #     out = File.read(File.join(tpm_dir, 'tpm_getpubek.txt'))
      #     expect(@tpm_fact.send(:get_pubek_owned, 'badpass',
      #       File.join(tpm_dir, 'mock_tpm_getpubek.rb'))).to eq out
      #   end
      # end
    end
    ...
    describe '.tpm_version' do
      it 'should return the value of the command' do
        out = File.read(File.join(tpm_dir, 'tpm_version.txt'))
    ...
    describe '.version' do
      context 'tpm_version exists' do
        before(:each) do
          @tpm_fact.stubs(:tpm_version).returns File.read(File.join(tpm_dir, 'tpm_version.txt'))
        end
    ...
    describe '.pubek' do
      ...
      context 'tpm is not owned' do
        before(:each) do
          @tpm_fact.stubs(:get_pubek_unowned).returns File.read(File.join(tpm_dir, 'tpm_getpubek.txt'))
        end
      ...
      context 'tpm is owned' do
        before(:each) do
          @tpm_fact.stubs(:get_pubek_owned).returns File.read(File.join(tpm_dir, 'tpm_getpubek.txt'))
