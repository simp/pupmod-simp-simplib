...
describe Puppet::Type.type(:runlevel).provider(:telinit) do
  let(:resource) { Puppet::Type.type(:runlevel).new(name: '5') }
  let(:provider) { Puppet::Type.type(:runlevel).provider(:telinit).new(resource) }
  ...
  context '#level=' do
    ...
    context 'with a timeout' do
      it 'raises an exception' do
        ...
        expect { provider.level = (resource[:level]) }.to raise_error(%r{Could not transition to runlevel})
      end
    end
    ...
    context 'persisting state' do
      let(:tempfile) do
        ...
        Tempfile.new(described_class.to_s)
      end
